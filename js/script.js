...
// Playful ball animations for light mode - Toy Story inspired
function initializeRandomSurprises() {
    // Only run surprises in light mode
    ...
    // More intentional timing patterns
    function triggerPlayfulBall() {
        ...
        const patterns = [
            // Single ball
            () => {
                showToyStoryBall(getNextBallType());
                scheduleNext(20000 + Math.random() * 15000);
            },
            // Double bounce (two balls from different sides)
            () => {
                showToyStoryBall(getNextBallType());
                setTimeout(() => showToyStoryBall(getNextBallType()), 1500);
                scheduleNext(25000 + Math.random() * 20000);
            },
            // Triple combo (three balls with staggered timing)
            () => {
                showToyStoryBall(getNextBallType());
                setTimeout(() => showToyStoryBall(getNextBallType()), 1000);
                setTimeout(() => showToyStoryBall(getNextBallType()), 2500);
                scheduleNext(30000 + Math.random() * 25000);
            },
            // Quick succession (rapid fire)
            () => {
                const ballType = getNextBallType();
                showToyStoryBall(ballType, { speed: 'fast' });
                setTimeout(() => showToyStoryBall(ballType, { speed: 'fast' }), 500);
                setTimeout(() => showToyStoryBall(ballType, { speed: 'fast' }), 1000);
                scheduleNext(35000 + Math.random() * 20000);
            },
            // Slow dramatic ball
            () => {
                showToyStoryBall(getNextBallType(), { speed: 'slow', dramatic: true });
                scheduleNext(40000 + Math.random() * 20000);
            }
        ];
        
        // Choose pattern based on sequence for variety
        const patternIndex = ballSequence % patterns.length;
        patterns[patternIndex]();
        ballSequence++;
    }
    
    function getNextBallType() {
        // Ensure we don't repeat the same ball type immediately
        let ballType;
        do {
            ballType = Math.floor(Math.random() * 6);
    ...
    function scheduleNext(delay) {
        setTimeout(triggerPlayfulBall, delay);
    }
    
    // Start the first ball sequence after page settles
    setTimeout(triggerPlayfulBall, 8000 + Math.random() * 5000);
}

function showToyStoryBall(ballTypeIndex = null, options = {}) {
    // Different ball styles like in Toy Story
    const ballTypes = [
        // Classic red ball with star
        {
        ...
        // Blue striped beach ball
        {
            background: 'repeating-conic-gradient(from 0deg at 50% 50%, #4dabf7 0deg 45deg, #ffffff 45deg 90deg)',
            size: 45,
            pattern: null
        },
        // Yellow smiley ball
        ...
        {
            background: 'conic-gradient(from 0deg at 50% 50%, #9c27b0, #4caf50, #9c27b0)',
            size: 38,
            pattern: null
        },
        ...
        // Rainbow ball
        {
            background: 'conic-gradient(from 0deg at 50% 50%, red, orange, yellow, green, blue, indigo, violet, red)',
            size: 40,
            pattern: null
        }
    ];
    
    // Use specific ball type or random
    ...
    const startY = options.dramatic ? 
        Math.random() * window.innerHeight * 0.3 : // Higher for dramatic effect
        Math.random() * window.innerHeight * 0.6;
    
    ball.style.cssText = `
        position: fixed;
        width: ${ballType.size}px;
        height: ${ballType.size}px;
        background: ${ballType.background};
        border-radius: 50%;
        ${startSide}: -60px;
        top: ${startY}px;
        z-index: 9999;
        ...
        box-shadow: 
            ...
            0 10px 20px rgba(0, 0, 0, ${options.dramatic ? 0.4 : 0.2});
        display: flex;
        align-items: center;
        justify-content: center;
        font-size: ${ballType.size * 0.5}px;
        animation: toyStoryBounce ${animationDuration}s cubic-bezier(0.25, 0.46, 0.45, 0.94) forwards;
        transform-origin: center;
    `;
    
    // Add pattern or lines if specified
    if (ballType.pattern) {
        ball.textContent = ballType.pattern;
        if (ballType.patternColor) {
            ball.style.color = ballType.patternColor;
        }
    }
    ...
    if (ballType.lines) {
        ...
        `;
        lines.innerHTML = `
            <div style="position: absolute; width: 100%; height: 2px; background: rgba(0,0,0,0.3); top: 50%; transform: translateY(-50%);"></div>
            <div style="position: absolute; width: 2px; height: 100%; background: rgba(0,0,0,0.3); left: 50%; transform: translateX(-50%);"></div>
        `;
    ...
    style.textContent = `
        @keyframes toyStoryBounce {
            ...
            5% { 
                transform: translateX(${direction * totalDistance * 0.1}px) translateY(-${bounceHeight}px) rotate(${direction * 90}deg) scaleY(1);
            }
            10% { 
                transform: translateX(${direction * totalDistance * 0.15}px) translateY(0) rotate(${direction * 180}deg) scaleY(0.8) scaleX(1.2);
            }
            11% {
                transform: translateX(${direction * totalDistance * 0.15}px) translateY(0) rotate(${direction * 180}deg) scaleY(1) scaleX(1);
            }
            20% { 
                transform: translateX(${direction * totalDistance * 0.3}px) translateY(-${bounceHeight * 0.8}px) rotate(${direction * 360}deg) scaleY(1);
            }
            28% { 
                transform: translateX(${direction * totalDistance * 0.4}px) translateY(0) rotate(${direction * 540}deg) scaleY(0.85) scaleX(1.15);
            }
            29% {
                transform: translateX(${direction * totalDistance * 0.4}px) translateY(0) rotate(${direction * 540}deg) scaleY(1) scaleX(1);
            }
            38% { 
                transform: translateX(${direction * totalDistance * 0.52}px) translateY(-${bounceHeight * 0.6}px) rotate(${direction * 720}deg) scaleY(1);
            }
            45% { 
                transform: translateX(${direction * totalDistance * 0.6}px) translateY(0) rotate(${direction * 900}deg) scaleY(0.9) scaleX(1.1);
            }
            46% {
                transform: translateX(${direction * totalDistance * 0.6}px) translateY(0) rotate(${direction * 900}deg) scaleY(1) scaleX(1);
            }
            55% { 
                transform: translateX(${direction * totalDistance * 0.72}px) translateY(-${bounceHeight * 0.4}px) rotate(${direction * 1080}deg) scaleY(1);
            }
            62% { 
                transform: translateX(${direction * totalDistance * 0.8}px) translateY(0) rotate(${direction * 1260}deg) scaleY(0.92) scaleX(1.08);
            }
            63% {
                transform: translateX(${direction * totalDistance * 0.8}px) translateY(0) rotate(${direction * 1260}deg) scaleY(1) scaleX(1);
            }
            72% { 
                transform: translateX(${direction * totalDistance * 0.9}px) translateY(-${bounceHeight * 0.2}px) rotate(${direction * 1440}deg) scaleY(1);
            }
            80% { 
                transform: translateX(${direction * totalDistance * 0.95}px) translateY(0) rotate(${direction * 1620}deg) scaleY(0.95) scaleX(1.05);
            }
            81% {
                transform: translateX(${direction * totalDistance * 0.95}px) translateY(0) rotate(${direction * 1620}deg) scaleY(1) scaleX(1);
            }
            90% {
                transform: translateX(${direction * totalDistance}px) translateY(-${bounceHeight * 0.1}px) rotate(${direction * 1800}deg) scaleY(1);
            }
            95% {
                transform: translateX(${direction * totalDistance * 1.05}px) translateY(0) rotate(${direction * 1980}deg) scaleY(0.97) scaleX(1.03);
            }
            100% { 
                transform: translateX(${direction * totalDistance * 1.1}px) translateY(0) rotate(${direction * 2160}deg) scaleY(1) scaleX(1);
            }
        }
    `;
    document.head.appendChild(style);
    ...
    // Add a subtle wobble animation during the bounce
    ball.addEventListener('animationstart', () => {
        ball.style.animation += ', ballWobble 0.5s ease-in-out infinite';
    });
    ...
    wobbleStyle.textContent = `
        @keyframes ballWobble {
            ...
            25% { transform-origin: 45% 55%; }
            75% { transform-origin: 55% 45%; }
        }
    `;
